...
var config = {
    ...
    $(document).on("click", ".tableRow", function(event){
        ...
        if($(this).attr("data-highlight")==="false"){
            ...
            $this.addClass("routStination");
            var named = removeArray.indexOf($(this).attr("data-name"))
            if(named != -1){
                removeArray.splice(named,1);
            };
    
        }
    })
        
    // how can I pass the snapshot into DBchanges?
    // OR
    // how can I have one function to handle all three: adding records, removing records, refreshing page?
    // trying to get the ETA to count down?
    
    database.ref("homework/").on("child_added", loadPage, errData);
    database.ref("homework/").on("child_removed", loadPage, errData);
    
    
    var trainsArray = [];
    
    function loadPage(snapshot){
        var snap = snapshot.val();
    
        trainsArray.push(snap);
